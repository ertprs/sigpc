...
function getNavElement(current){
    ...
    element = "";
    element += '<li><a href="' + current.find(">a").attr("href") + '" onClick="' + current.find(">a").attr("onClick") + '">' + currentText + "</a>";
...
/* elementos de guia UI busca*/
function expandeElemento(v_elemento,v_acao) {
   if(v_acao != "" && v_acao != null){ v_acaoi = v_acao; }else{ v_acaoi = "0"; }
   var $el = $('#'+v_elemento),
   content = $el.parents('.box').find(".box-content-ax");
   contentLeg = $el.parents('.box').find(".status-leg");
...
$(document).ready(function () {
	...
	//toggle-mobile
	//esconder menu mobile ao selecionar um link valido
	$('body').on('click',"#navigation .mobile-nav a", function(e){
		if($(this).attr('onclick') != "undefined"){ $(".toggle-mobile").click(); }
	});
...
$(".theme-colors > li > span").hover(function(e){
 ...
 setCookieF('CONF_BODY_COLOR',"theme-"+$el.attr("class"));
});
