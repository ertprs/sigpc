...
function contagem_datas($data1, $data2, $time='1', $tipo='1'){
	...
	if($dias > "1"){ $dias_leg = $class_fLNG->txt(__FILE__,__LINE__,'dias'); }else{ $dias_leg = $class_fLNG->txt(__FILE__,__LINE__,'dia'); } 
	if($horas > "1"){ $horas_leg = $class_fLNG->txt(__FILE__,__LINE__,'horas'); }else{ $horas_leg = $class_fLNG->txt(__FILE__,__LINE__,'hora'); }
	if($minutos > "1"){ $minutos_leg = $class_fLNG->txt(__FILE__,__LINE__,'minutos'); }else{ $minutos_leg = $class_fLNG->txt(__FILE__,__LINE__,'minuto'); }
	if(($dias<="0") and ($horas<="0") and ($minutos<="0")){
		$var_r = $class_fLNG->txt(__FILE__,__LINE__,'completado');
	}
	if($tipo == "1"){
		$var_r = "";
		if($dias != "0"){
...
function calcular_idade($nascimento,$adiciona='0') {
	...
	if($adiciona == "1"){ $adiciona = $idade;
		if($adiciona == "0"){ $idade .= " ".$class_fLNG->txt(__FILE__,__LINE__,'ano'); }
		if($adiciona == "1"){ $idade .= " ".$class_fLNG->txt(__FILE__,__LINE__,'ano'); }
		if($adiciona >= "2"){ $idade .= " ".$class_fLNG->txt(__FILE__,__LINE__,'anos'); }
	}
...
function difHoraTime($time_analizar, $time_dif='0',$ponteiro="1",$legenda_ax="atrás"){
	...
	$imprime = $class_fLNG->txt(__FILE__,__LINE__,'NUNCA');	
			...
			if($result > "24"){
				...
				if($result > "7"){
					...
					if($result > "3"){
						...
						$imprime = $result." ".$class_fLNG->txt(__FILE__,__LINE__,'semanas(s)')." ".$legenda_ax;
						$oqueM = "1";
					}
				}else{
					$imprime = $result." ".$class_fLNG->txt(__FILE__,__LINE__,'dia(s)')." ".$legenda_ax;
					$oqueM = "1";
				}
			}else{
				$imprime = $result." ".$class_fLNG->txt(__FILE__,__LINE__,'hora(s)')." ".$legenda_ax;
				$oqueM = "0";
			}
		}else{
			$imprime = $result." ".$class_fLNG->txt(__FILE__,__LINE__,'minuto(s)')." ".$legenda_ax;
			$oqueM = "0";
		}
		$imprime_ = $imprime;
...
function msgDisplay($MSG){
	...
	if((isset($MSG["SUCESSO"])) and ($MSG["SUCESSO"] != "")){
		echo "<script>$.doTimeout('vTimerMsgs', 500, function(){ exibMensagem('".$divADD."','sucesso','<i class=\"icon-info-sign\"></i> <b>".$class_fLNG->txt(__FILE__,__LINE__,'SUCESSO')."!</b><br>".$MSG["SUCESSO"]."'); });</script>";
	}//SUCESSO
	if((isset($MSG["INFO"])) and ($MSG["INFO"] != "")){
		echo "<script>$.doTimeout('vTimerMsgi', 500, function(){ exibMensagem('".$divADD."','info','<i class=\"icon-info-sign\"></i> <b>".$class_fLNG->txt(__FILE__,__LINE__,'INFO')."!</b><br>".$MSG["INFO"]."');});</script>";
	}//INFO
	if((isset($MSG["ERRO"])) and ($MSG["ERRO"] != "")){
		echo "<script>$.doTimeout('vTimerMsge', 500, function(){ exibMensagem('".$divADD."','erro','<i class=\"icon-info-sign\"></i> <b>".$class_fLNG->txt(__FILE__,__LINE__,'ERRO')."!</b><br>".$MSG["ERRO"]."');});</script>";
	}//ERRO
	if((isset($MSG["ALERTA"])) and ($MSG["ALERTA"] != "")){
		echo "<script>$.doTimeout('vTimerMsga', 500, function(){ exibMensagem('".$divADD."','alerta','<i class=\"icon-info-sign\"></i> <b>".$class_fLNG->txt(__FILE__,__LINE__,'ALERTA')."!</b><br>".$MSG["ALERTA"]."');});</script>";
	}//ALERTA
}
...
/*FUNÇÃO  PARA RETORNO DE LEGENDA PERMISSOES EXTRAS
*/
function legPermissoesExtras($var){
	global $class_fLNG;//DECLARAÇÃO DE CLASSE fLNG GLOBAL
	$leg = $class_fLNG->txt(__FILE__,__LINE__,'CANCELADO');
	if($var == "edi"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Editar'); }
	if($var == "cad"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Cadastrar'); }
	if($var == "exc"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Excluir'); }
	
	if($var == "gra"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Gráficos'); }
	if($var == "exp"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Exportar'); }
	
	if($var == "tds"){ $leg = "Visualizar Todos os Registros"; }
	if($var == "res"){ $leg = "Alterar Responsável"; }
	if($var == "ajc"){ $leg = "Acessar Já Cancelados"; }
	if($var == "ajf"){ $leg = "Acessar Já Finalizados"; }
	if($var == "vip"){ $leg = "Visualizar Todas Unidades"; }
	if($var == "dep"){ $leg = "Visualizar Todos Deptos"; }
	if($var == "per"){ $leg = "Alterar unidade (permite criar/alterar outros perfils)"; }
	if($var == "vt3"){ $leg = "Visualizar do Perfil Atual"; }
	if($var == "vt4"){ $leg = "Visualizar de Todos Perfils"; }
	if($var == "edg"){ $leg = "Add/Editar Grupos"; }
	if($var == "fdp"){ $leg = "Visualização Completa (fora do depto)"; }
	if($var == "aud"){ $leg = "Analisar Deduplicação"; }
	if($var == "obs"){ $leg = "Governo - Analisar ABIS"; }	
	return $leg;
}//legPermissoesExtras
...
function legSexo($var,$acao="min"){
	global $class_fLNG;//DECLARAÇÃO DE CLASSE fLNG GLOBAL
	$sexo_leg = $class_fLNG->txt(__FILE__,__LINE__,'Indefinido');	
	if(($var == "M") or ($var == "m") or ($var == "1")){ $sexo_leg = $class_fLNG->txt(__FILE__,__LINE__,'Masculino'); }
	if(($var == "F") or ($var == "f") or ($var == "2")){ $sexo_leg = $class_fLNG->txt(__FILE__,__LINE__,'Feminino'); }
...
function legAcaoWebservice($var,$acao="id"){
	...
	if($acao == "leg"){//retorna a legenda da tabela ao web service
		...
		if(($var == "sys_webservice_dados") or ($var == "2")){ $leg = "Web Service SIGPC de Dados"; }
	}
	return $leg;
}//legAcaoWebservice
...
function categoriaServicoArr($tipo_pessoa = ""){
	if($tipo_pessoa == "" || $tipo_pessoa == "fisico"){
		//$ARR = array("habilitacao","veiculo","reclamacao","info","coleta","retirada");
		$ARR = array("habilitacao","reclamacao","info","coleta","retirada");
	}else{//if($tipo_pessoa == "" || $tipo_pessoa == "fisico"){
		//$ARR = array("veiculo","reclamacao","info");
		$ARR = array("reclamacao","info");
	}//}else{//if($tipo_pessoa == "" || $tipo_pessoa == "fisico"){
	return $ARR;
}//categoriaServicoArr
...
function categoriaServicoLeg($tipo){
	global $class_fLNG;//DECLARAÇÃO DE CLASSE fLNG GLOBAL
	$leg = "";
	if($tipo == "habilitacao"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Habilitação'); }
	if($tipo == "veiculo"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Veículo'); }	
	if($tipo == "reclamacao"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Reclamação'); }		
	if($tipo == "info"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Informações'); }			
	if($tipo == "retirada"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Retirada'); }				
	if($tipo == "coleta"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Coleta Biométrica'); }				
	return $leg;
}//categoriaServicoLeg
...
function categoriaServicoIco($tipo){
	...
	if($tipo == "reclamacao"){ $icon = "icon-pencil"; }		
	if($tipo == "info"){ $icon = "icon-info-sign"; }			
	if($tipo == "retirada"){ $icon = "icon-share"; }				
	if($tipo == "coleta"){ $icon = "fas fa-fingerprint"; }
	return $icon;
}//categoriaServicoIco

function categoriaServicoIDSENHA($tipo){
	$id = "";
	if($tipo == "habilitacao"){ $id = "1"; }
	if($tipo == "veiculo"){ $id = "2"; }	
	if($tipo == "reclamacao"){ $id = "3"; }		
	if($tipo == "info"){ $id = "4"; }			
	if($tipo == "coleta"){ $id = "5"; }				
	if($tipo == "retirada"){ $id = "6"; }	
	return $id;
}//categoriaServicoLeg


function categoriaServicoArrCompleto($tipo){
	global $class_fLNG;//DECLARAÇÃO DE CLASSE fLNG GLOBAL
	$ARR = array();
	
	$row["tipo"] = "habilitacao";
	$row["leg"] = $class_fLNG->txt(__FILE__,__LINE__,'Habilitação');
	$row["ico"] = "glyphicon-vcard";
	$row["status"] = "1";	
	$ARR["1"] = $row;

	/*
	$row["tipo"] = "veiculo";
	$row["leg"] = $class_fLNG->txt(__FILE__,__LINE__,'Veículo');
	$row["ico"] = "glyphicon-car";
	$row["status"] = "0";		
	$ARR["2"] = $row;
	*/
	$row["tipo"] = "reclamacao";
	$row["leg"] = $class_fLNG->txt(__FILE__,__LINE__,'Reclamação');
	$row["ico"] = "icon-pencil";
	$row["status"] = "0";		
	$ARR["3"] = $row;
	
	$row["tipo"] = "info";
	$row["leg"] = $class_fLNG->txt(__FILE__,__LINE__,'Informações');
	$row["ico"] = "icon-info-sign";
	$row["status"] = "1";		
	$ARR["4"] = $row;
	
	$row["tipo"] = "coleta";
	$row["leg"] = $class_fLNG->txt(__FILE__,__LINE__,'Coleta Biométrica');
	$row["ico"] = "fas fa-fingerprint";
	$row["status"] = "0";		
	$ARR["5"] = $row;
	
	$row["tipo"] = "retirada";
	$row["leg"] = $class_fLNG->txt(__FILE__,__LINE__,'Retirada');
	$row["ico"] = "icon-share";
	$row["status"] = "1";		
	$ARR["6"] = $row;	
...
function processoStatusLeg($status){
	...
	if($status == "999"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'AGUARDANDO LIBERAÇÃO (CADAC)'); }				
	return $leg;
}//processoStatusLeg
...
function processoTipoProcesso($servico_id){
	...
	if($servico_id == "19"){ $cod = (int)"5"; }//PID
	return $cod;
...
function docPessoaFisica($candidato_id){
	...
	if($linha2["rg"] != "" and $linha2["rg_data"] != ""){ 
		$ARR["doc"] = "1";
		$ARR["nome"] = $class_fLNG->txt(__FILE__,__LINE__,'identidade');		
	...
	if($ARR["doc"] == ""){
		if(($linha2["passaporte"] != "") and ($linha2["passaporte_data"] != "") and ($linha2["passaporte_pais"] != "")){
			$ARR["doc"] = "2";
			$ARR["nome"] = $class_fLNG->txt(__FILE__,__LINE__,'passaporte');
	...
	if($ARR["doc"] == ""){
		if(($linha2["id_estrangeiro"] != "") and ($linha2["id_estrangeiro_data"] != "")){
			$ARR["doc"] = "3";
			$ARR["nome"] = $class_fLNG->txt(__FILE__,__LINE__,'ID Estrangeiro');
...
function thomasFiltrarErro($string){
	global $class_fLNG;
	$msg = "";
	return " - ".$string;
	if((preg_match("/$/i",$string)) and ($string != "")){
		...
		foreach($array as $campo => $arr){
			if($campo == "numero_rnt"){ if($msg != ""){ $msg .= "<br>"; } $msg .= $class_fLNG->txt(__FILE__,__LINE__,'RNT do candidato é obrigatório!, verifique!'); }
			if($campo == "numero_processo"){ if($msg != ""){ $msg .= "<br>"; } $msg .= $class_fLNG->txt(__FILE__,__LINE__,'RNC é obrigatório, verifique!'); }		
			if($campo == "id_origem"){ if($msg != ""){ $msg .= "<br>"; } $msg .= $class_fLNG->txt(__FILE__,__LINE__,'Unidade de origem é obrigatório, verifique!'); }		
			if($campo == "tipo_processo"){ if($msg != ""){ $msg .= "<br>"; } $msg .= $class_fLNG->txt(__FILE__,__LINE__,'Tipo de processo não foi definido, verifique!'); }		
			if($campo == "tipo_coleta"){ if($msg != ""){ $msg .= "<br>"; } $msg .= $class_fLNG->txt(__FILE__,__LINE__,'Tipo de coleta não foi definido, verifique!'); }		
			if($campo == "nome"){ if($msg != ""){ $msg .= "<br>"; } $msg .= $class_fLNG->txt(__FILE__,__LINE__,'Nome do candidato é obrigatório, acesso o cadastro e verifique!'); }		
			if($campo == "sobrenome"){ if($msg != ""){ $msg .= "<br>"; } $msg .= $class_fLNG->txt(__FILE__,__LINE__,'Sobrenome do candidato é obrigatório, acesso o cadastro e verifique!'); }		
			if($campo == "sexo"){ if($msg != ""){ $msg .= "<br>"; } $msg .= $class_fLNG->txt(__FILE__,__LINE__,'Gênero do candidato é obrigatório, acesso o cadastro e verifique!'); }		
			if($campo == "nacionalidade"){ if($msg != ""){ $msg .= "<br>"; } $msg .= $class_fLNG->txt(__FILE__,__LINE__,'Nacionalidade do candidato deve ser informada, verifique!'); }		
			if($campo == "local_nascimento"){ if($msg != ""){ $msg .= "<br>"; } $msg .= $class_fLNG->txt(__FILE__,__LINE__,'Local de nascimento do candidato deve ser informado, verifique!'); }		
			if($campo == "data_nascimento"){ if($msg != ""){ $msg .= "<br>"; } $msg .= $class_fLNG->txt(__FILE__,__LINE__,'Data de nascimento não foi informada, acesso o cadastro e verifique!'); }		
			if($campo == "doc_tipo"){ if($msg != ""){ $msg .= "<br>"; } $msg .= $class_fLNG->txt(__FILE__,__LINE__,'Tipo de documento utilizado não foi definido, acesse o cadastro e verifique!'); }		
			if($campo == "doc_numero"){ if($msg != ""){ $msg .= "<br>"; } $msg .= $class_fLNG->txt(__FILE__,__LINE__,'Nº do documento pessoal do candidato não foi informado, acesse o cadastro e verifique!'); }		
			if($campo == "doc_data_emissao"){ if($msg != ""){ $msg .= "<br>"; } $msg .= $class_fLNG->txt(__FILE__,__LINE__,'Data de emissão do documento pessoal não foi informada, acesse o cadastro e verifique!'); }		
			if($campo == "codigo_motivo"){ if($msg != ""){ $msg .= "<br>"; } $msg .= $class_fLNG->txt(__FILE__,__LINE__,'Motivo de cancelamento não foi informado, verifique!'); }		
			if($campo == "motivo_cancelamento"){ if($msg != ""){ $msg .= "<br>"; } $msg .= $class_fLNG->txt(__FILE__,__LINE__,'Motivo de cancelamento não foi informado, verifique!'); }		
			if($campo == "data_emissao_habilitacao"){ if($msg != ""){ $msg .= "<br>"; } $msg .= $class_fLNG->txt(__FILE__,__LINE__,'Data de emissão da habilitação é obrigatório!'); }		
			if($campo == "data_validade_habilitacao"){ if($msg != ""){ $msg .= "<br>"; } $msg .= $class_fLNG->txt(__FILE__,__LINE__,'Data de validade da habilitação é obrigatório!'); }		
			if($campo == "uf_habilitacao"){ if($msg != ""){ $msg .= "<br>"; } $msg .= $class_fLNG->txt(__FILE__,__LINE__,'UF da habilitação é obrigatório!'); }		
			if($campo == "categorias"){ if($msg != ""){ $msg .= "<br>"; } $msg .= $class_fLNG->txt(__FILE__,__LINE__,'Categorias da habilitação não foi definida, verifique!'); }		
			if($campo == "categoria"){ if($msg != ""){ $msg .= "<br>"; } $msg .= $class_fLNG->txt(__FILE__,__LINE__,'Categoria da habilitação não foi definida, verifique!'); }		
			if($campo == "data_aquisicao"){ if($msg != ""){ $msg .= "<br>"; } $msg .= $class_fLNG->txt(__FILE__,__LINE__,'Data de aquisição da categoria não foi informada!'); }		
			if($campo == "id_problema"){ if($msg != ""){ $msg .= "<br>"; } $msg .= $class_fLNG->txt(__FILE__,__LINE__,'Tipo de problema não foi informado, verifique!'); }		
			if($campo == "acao_problema"){ if($msg != ""){ $msg .= "<br>"; } $msg .= $class_fLNG->txt(__FILE__,__LINE__,'Ação a ser executada não foi definida, verifique!'); }		
		}//fim foreach
...
function thomasWS($metodo, $url, $dados,$get=""){
	$url = $url.$metodo.$get;
	//echo $url;
	if($get == ""){
	...
	//echo "<br>contentsBin:".$contentsBin;
	//file_put_contents(time()."_DEBUG.txt",$contentsBin);
...
function thomasCodRetorno($cod){
	global $class_fLNG;
	$leg = "";
	if($cod == "1"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Processado'); }
	if($cod == "2"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Falha no Processamento'); }
	if($cod == "3"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Dados Incompletos'); }
	if($cod == "4"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Impresso'); }
	if($cod == "5"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Falha na Impressão'); }
	if($cod == "6"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Informação Desalinhada'); }
	return $leg;
}//thomasCodRetorno



function thomasWScoletaBiometricaEnviar($processo_id,$origem_id="",$usuario_id=""){
	global $class_fLNG;
	...
	if ($origem_id == "") { $origem_id = $linha["origem_id"]; }
	...
	if($result["codigo_retorno"] == "1"){ 
		...
		if($motivo_id_a >= "1"){
			fSQL::SQL_UPDATE_SIMPLES("resolvido,user_a,sync","axl_motivo_cancelamento",array("1",time(),$usuario_id,time()),"id = '".$motivo_id_a."'");	
		}//if($motivo_id_a >= "1"){
			
		//redirecionar senha
		if($tipo_coleta_a == "2"){
			$linha = fSQL::SQL_SELECT_ONE("senha","axl_triagem","origem_id = '$origem_id' AND code = '$code_a'");
			$acao_servico_id = categoriaServicoIDSENHA("habilitacao");
			$acao_redirecionar_servico_id = categoriaServicoIDSENHA("coleta");				
			fSENHA::acao($usuario_id,$origem_id,$acao_servico_id,$linha["senha"],"7",$acao_redirecionar_servico_id);
		}//if($tipo_coleta_a == "2"){			
	}//if($result["codigo_retorno"] == "1"){ 
...
function thomasWSimpressaoEnviar($processo_id,$origem_id='0'){
	global $class_fLNG;
	$linha = fSQL::SQL_SELECT_ONE("code,ano,mes,dia,origem_id,servico_id,candidato_fisico_id,candidato_juridico_id,motivo_id,validade_anos,validade_time,coleta_id","axl_processo","id = '".$processo_id."'");
	$code_a = $linha["code"];
	$ano = $linha["ano"];
	$mes = $linha["mes"];
	...
	if ($origem_id == '0'){ $origem_id = $linha["origem_id"]; }
	...
	$restricoes = ""; $arrCategorias = array();
	...
	if($restricoes != ""){ $restricoes = str_replace("[,","",$restricoes); $restricoes = str_replace(",]","",$restricoes); }
	...
	$arrDados["id_origem"] = (int)$origem_id;
	$arrDados["descricao_origem"] = $origem_descricao;	
	$arrDados["tipo_processo"] = (int)$tipo_processo;
	$arrDados["nome"] = $linha2["nome"];		
	$arrDados["sobrenome"] = $linha2["sobrenome"];		
	$arrDados["sexo"] = ($linha2["sexo"] == 2) ? 1 : 0;				
	...
	//echo "dados: <pre>"; print_r($arrDados); echo "</pre>";		
	//file_put_contents(time()."_DEBUG-dados.txt",json_encode($arrDados));
	$result = thomasWS("gravarAutorizacaoEmissaoDocumento",WS_URL_BIOMETRIA,$arrDados);
	//echo "result: <pre>"; print_r($result); echo "</pre>";		
	//file_put_contents(time()."_DEBUG-resultws.txt",json_encode($result));
	//atualizar processo
	if($result["codigo_retorno"] == "1"){ 
		fSQL::SQL_UPDATE_SIMPLES("status,sync,motivo_id,emissao_time,validade_time","axl_processo",array("3",time(),"0",$emissao_time_a,$validade_time_a),"id = '".$processo_id."'");
		...
		if($linhaxxx["email"] != ""){
			...
			$opts = array(
				'http'=>array('header'=>"Content-type: application/x-www-form-urlencoded", 'method'=>'POST', 'content'=>http_build_query(array(
					...
					'SEND_NOME' => primeiro_nome($linha2["nome"]),
					'SEND_EMAIL' => $email_a,
					'SEND_ASSUNTO' => $class_fLNG->txt(__FILE__,__LINE__,'!!nome!!, Pré-Cadastro Concluído',array("nome"=>primeiro_nome($linha2["nome"]))),
					'SEND_BODY' => $html_template
					))),"ssl"=>array("verify_peer"=>false,"verify_peer_name"=>false)
			); $context = stream_context_create($opts);
			//informações do retorno: 0 - bloqueado acesso, 1 - sucesso, 2 - erros de configuração, 3 - erro de classe
...
function thomasWSabisProblema($id_problema){
	$result = thomasWS("consultarProblemaABIS",WS_URL_BIOMETRIA,"","?id_problema=".$id_problema);
	
	return $result;		
}//thomasWSabisProblemas




function thomasWSabisProblemaResolver($id_problema, $acao){
	unset($arrDados); $arrDados = array();
	$arrDados["id_problema"] = (int)$id_problema;
	$arrDados["acao_problema"] = (int)$acao;	

	$result = thomasWS("resolverProblemaABIS",WS_URL_BIOMETRIA,$arrDados);
...
function legDedo($posicao){
	global $class_fLNG;
	$leg = "";
	if($posicao == "1"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Polegar'); }
	elseif($posicao == "2"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Indicador'); }
	elseif($posicao == "3"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Médio'); }
	elseif($posicao == "4"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Anular'); }
	elseif($posicao == "5"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Mínimo'); }
	elseif($posicao == "6"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Polegar'); }
	elseif($posicao == "7"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Indicador'); }	
	elseif($posicao == "8"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Médio'); }	
	elseif($posicao == "9"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Anular'); }
	elseif($posicao == "10"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Mínimo'); }		
...
function legPrioridade($prioridade){
	global $class_fLNG;
	$leg = "";
	if($prioridade == "1"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Normal'); }else{ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Prioridade'); }
	return $leg;	
}//legPrioridade

function legABIS($status){
	global $class_fLNG;
	$leg = "";
	if($status == "1"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'OK'); }
	if($status == "2"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Duplicidade'); }
	if($status == "3"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Incompatível'); }	
	if($leg == ""){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Indefinido'); }
	return $leg;	
}//legPrioridade

function legABISacao($acao){
	global $class_fLNG;
	$leg = "";
	if($acao == "1"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'NoMatch'); }
	if($acao == "2"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Atualizar'); }
	if($acao == "3"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Rejeitar'); }	
	return $leg;	
}//legPrioridade
...
function legEtapaCancelamento($etapa){
	global $class_fLNG;
	$leg = "";
	if($etapa == "1"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Coleta Biométrica'); }
	if($etapa == "2"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Gráfica'); }
	if($etapa == "3"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Entrega'); }
	return $leg;	
}//legEtapaCancelamento





function processoDetalhesLeg($processo_id,$INC_FAISHER="",$faisher=""){
	global $class_fLNG;
	$linha = fSQL::SQL_SELECT_ONE("P.code,P.tipo_servico,P.origem_id,P.servico_id,P.validade_time,P.status,P.emissao_id,P.time,P.motivo_id,P.candidato_fisico_id,T.nome as servico_nome","axl_processo P, adm_protocolo_tipo T","P.id = '".$processo_id."' AND P.servico_id = T.id");

	$historico_n = '<span class="display">'.SYS_CONFIG_PROCESSO_SIGLA.' '.$linha["code"].' - '.$linha["servico_nome"].'</span>';	
	//$historico_n .= '<button type="button" class="btn" onclick="pmodalHtml(\'<i class=icon-search></i> DETALHES DO PROCESSO #'.$linha["code"].'\',\''.$AJAX_PAG.'\',\'get\',\'faisher='.$faisher.'&ajax=detalhesProcesso&id='.$processo_id.'\');return false;"><i class="icon-search"></i></button>';
	$historico_n .= ' <span class="label">'.processoStatusLeg($linha["status"]).'</span>';	
	
	//se houver algum cancelamento na coleta, retornar para atendimento
	if($linha["motivo_id"] >= "1" and $INC_FAISHER != ""){
		$linha2 = fSQL::SQL_SELECT_ONE("sync","cad_candidato_fisico","id = '".$linha["candidato_fisico_id"]."'");
		$pessoa_sync = $linha2["sync"];
		
		$linha2 = fSQL::SQL_SELECT_ONE("descricao,time","axl_motivo_cancelamento","id = '".$linha["motivo_id"]."'");
		$motivo_time = $linha2["time"];
		$motivo_descricao = $linha2["descricao"];			
		$css_color = "red";
		if($motivo_time < $pessoa_sync){ 
			$historico_n .= '<br><span style="color:green;">*'.$class_fLNG->txt(__FILE__,__LINE__,'RESOLVIDO ERRO NA COLETA BIOMÉTRICA').'<br><i>'.$motivo_descricao.'</i></span>';
			$historico_n .= ' <a href="#" class="btn btn-primary btn-xs" onclick="enviarColeta'.$INC_FAISHER.'();return false;"><i class="glyphicon-ok_2"></i> '.$class_fLNG->txt(__FILE__,__LINE__,'Enviar para coleta').'</a></span>';
		}else{//if($motivo_time < $pessoa_sync){ 
			$historico_n .= '<br><span style="color:red;">*'.$class_fLNG->txt(__FILE__,__LINE__,'ERRO NA COLETA BIOMÉTRICA').'<br><i>'.$motivo_descricao.'</i></span>';
			$historico_n .= ' <a href="#" class="btn btn-red btn-xs" onclick="popupCandidatofisico'.$INC_FAISHER.'(\''.$linha["candidato_fisico_id"].'\');return false;"><i class="glyphicon-warning_sign"></i> '.$class_fLNG->txt(__FILE__,__LINE__,'Resolver').'</a></span>';
		}//}else{//if($motivo_time < $pessoa_sync){ 	
	}else{//if($linha["motivo_id"] >= "1"){
		/*
		$historico_n .= "<br>";
		if($linha["emissao_id"] != ""){ $historico_n .= 'Nº Permissão '.$linha["emissao_id"]; }
		
		if($linha["servico_id"] >= "13" and $linha["servico_id"] <= "15"){
			//montar categorias
			$categorias = "";
			$resu = fSQL::SQL_SELECT_SIMPLES("tipo_id,tipo_campo,nome,valor,validade_time,validade_anos","axl_processo_campos","processo_id = '".$processo_id."' and tipo_campo = '99'");
			while($linha2 = fSQL::FETCH_ASSOC($resu)){
				if($linha2["tipo_campo"] == "99"){//CATEGORIAS
					if($categorias != ""){ $categorias .= ", "; }
					$categorias .= $linha2["nome"];
				}//if($linha["tipo_campo"] == "99"){//CATEGORIAS
			}//fim while
			if($linha["emissao_id"] != ""){ $historico_n .= ", "; }
			$historico_n .= "categoria(s): ".$categorias;
		}//if($linha["servico_id"] >= "13" and $linha["servico_id"] >= "15"){
		*/
	}
	$historico_n .= "<br><small><i>".$class_fLNG->txt(__FILE__,__LINE__,'Aberto em')." ".date("d/m/Y",$linha["time"]);
	if($linha["origem_id"] == 9){
		$historico_n .= " <b>(".$class_fLNG->txt(__FILE__,__LINE__,'EXTERNO').")</b></i></small>";
	}
	if($linha["validade_time"] >= "1"){ $historico_n .= "<small><i>, ".$class_fLNG->txt(__FILE__,__LINE__,'válido até')." ".date("d/m/Y",$linha["validade_time"])."</i></small>"; }

	return $historico_n;
}


function legBanco($acao){
	global $class_fLNG;
	$leg = "";
	if($acao == "1"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Ecobank Guinée (EBG)'); }
	if($acao == "2"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Skye Bank Guinée (SBG)'); }
	if($acao == "3"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Orabank Guinée (ORANBANK)'); }	
	if($acao == "4"){ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Banque Islamique de Guinée (BIG)'); }		
	return $leg;	
}//legPrioridade
...
//monta legenda de status das verificações de credenciamento: sys_login
function legVerificacaoCredenciamento($var){
	global $class_fLNG;
	$ret = $var;
	//ativo
	$arr["4"] = $class_fLNG->txt(__FILE__,__LINE__,'NEGADO');
	$arr["1"] = $class_fLNG->txt(__FILE__,__LINE__,'ACEITO');
	$arr["3"] = $class_fLNG->txt(__FILE__,__LINE__,'AGUARDANDO VERIFICAÇÃO');
	$arr["5"] = $class_fLNG->txt(__FILE__,__LINE__,'REVOGADO');
	if($ret != "array"){ if(isset($arr["$ret"])){ $leg = $arr["$ret"]; }else{ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Indefinido'); } }else{ $leg = $arr; }
	return $leg;
}//legAppVerificacaoStatus


function acaoProcessoLeg($var){
	global $class_fLNG;
	$ret = $var;
	//ativo
	$arr["1"] = $class_fLNG->txt(__FILE__,__LINE__,'suspensão');
	$arr["2"] = $class_fLNG->txt(__FILE__,__LINE__,'cancelamento');	
	if($ret != "array"){ if(isset($arr["$ret"])){ $leg = $arr["$ret"]; }else{ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Indefinido'); } }else{ $leg = $arr; }
	return $leg;	
}

//monta legenda de status das verificações de credenciamento: sys_login
function motivoCancelamento($var){
	global $class_fLNG;
	$ret = $var;
	//ativo
	$arr["4"] = $class_fLNG->txt(__FILE__,__LINE__,'Solicitação do governo');
	//$arr["1"] = $class_fLNG->txt(__FILE__,__LINE__,'Erro nos dados biométricos');
	//$arr["2"] = $class_fLNG->txt(__FILE__,__LINE__,'Erro nos dados biográficos');
	//$arr["3"] = $class_fLNG->txt(__FILE__,__LINE__,'Erro nos dados biométricos e biográficos ');
	if($ret != "array"){ if(isset($arr["$ret"])){ $leg = $arr["$ret"]; }else{ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Indefinido'); } }else{ $leg = $arr; }
	return $leg;
}//legAppVerificacaoStatus


function legSistema($var){
	global $class_fLNG;
	$ret = $var;	
	$arr["1"] = $class_fLNG->txt(__FILE__,__LINE__,'Sistema de Captura');
	$arr["90130"] = $class_fLNG->txt(__FILE__,__LINE__,'Sistema de Impressão');
	$arr["90131"] = $class_fLNG->txt(__FILE__,__LINE__,'Sistema de Entrega');
	$arr["90132"] = $class_fLNG->txt(__FILE__,__LINE__,'Aplicativo de Consulta (QRCode)');	
	if($ret != "array"){ if(isset($arr["$ret"])){ $leg = $arr["$ret"]; }else{ $leg = $class_fLNG->txt(__FILE__,__LINE__,'Indefinido'); } }else{ $leg = $arr; }
	return $leg;
}
...
function enviarEmailCandidato($candidato_id){
	global $class_fLNG;
	$linha = fSQL::SQL_SELECT_ONE("id,email","cad_candidato_fisico_email","candidato_fisico_id = '".$candidato_id."' AND principal = '1'");
	if($linha["id"] >= "1"){
		$email = $linha["email"];

		$arrCandidato = fSQL::SQL_SELECT_ONE("*","cad_candidato_fisico","id = '".$candidato_id."'");
		$arrEnd = fSQL::SQL_SELECT_ONE("*","cad_candidato_fisico_endereco","id = '".$candidato_id."'");
		
		$notificacao = "<span style='font-size:14px;'>".$class_fLNG->txt(__FILE__,__LINE__,'Seu cadastro foi realizado no SIGPC!')."</span>";
		...
		$opts = array(
			'http'=>array('header'=>"Content-type: application/x-www-form-urlencoded", 'method'=>'POST', 'content'=>http_build_query(array(
				...
				'SEND_EMAIL' => $email,
				'SEND_ASSUNTO' => $class_fLNG->txt(__FILE__,__LINE__,'Cadastro Concluído')." ".primeiro_nome($arrCandidato["nome"]),
				'SEND_BODY' => $html_template
				))),"ssl"=>array("verify_peer"=>false,"verify_peer_name"=>false)
		); $context = stream_context_create($opts);
		//informações do retorno: 0 - bloqueado acesso, 1 - sucesso, 2 - erros de configuração, 3 - erro de classe
...
function capaProcesso($code, $qrcode_img, $inc = '../') {
	$qrcode = file_get_contents('https://chart.googleapis.com/chart?chs=100x100&cht=qr&chl='.$code.'');
	//$qrcode_img = time().".jpeg";
	file_put_contents(VAR_DIR_FILES.'files/temp/'.$qrcode_img, $qrcode);
	
	$arProcesso = fSQL::SQL_SELECT_ONE("C.code AS candidato_code,C.nome AS candidato_nome,S.nome AS servico_nome,P.tipo_servico,P.externo,P.cfc,P.medico","axl_processo P, cad_candidato_fisico C, adm_protocolo_tipo S","P.candidato_fisico_id = C.id AND P.servico_id = S.id AND P.code = '".$code."'");
	
	if ($arProcesso["externo"] <= "0"){
		$cfc = "Livrer un document physique";
		$medico = "Livrer un document physique";
	} else {
		$cfc = $arProcesso["cfc"];
		$medico = $arProcesso["medico"];
	}
	
	$espacos = '&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp';
	$html = "";
	$html .= "<div style='margin-top:150px; text-align:center; font-size:18px;'><b>DÉPÔT DES DOCUMENTS DE LA LICENCE DE PROCESSUS<br>D'OBTENTION POUR CONDUIRE ENSEMBLE DANS DNTT</b></div>";
	$html .= "<span style='font-size:14px;'>";
	$html .= "<br><br>".$espacos."<b><img src='".VAR_DIR_RAIZ."img/arrow-right.png'/> Intéressé:</b> ".$arProcesso["candidato_nome"];
	$html .= "<br><br>".$espacos."<b><img src='".VAR_DIR_RAIZ."img/arrow-right.png'/> ".SYS_CONFIG_RM_SIGLA.":</b> ".$arProcesso["candidato_code"];
	$html .= "<br><br>".$espacos."<b><img src='".VAR_DIR_RAIZ."img/arrow-right.png'/> ".SYS_CONFIG_PROCESSO_SIGLA.":</b> ".$code;
	$html .= "<br><br>".$espacos."<b><img src='".VAR_DIR_RAIZ."img/arrow-right.png'/> Type de Processus:</b> ".$arProcesso["servico_nome"];
	$html .= "<br><br>".$espacos."<b><img src='".VAR_DIR_RAIZ."img/arrow-right.png'/> Auto-école:</b> ".$cfc;
	$html .= "<br><br>".$espacos."<b><img src='".VAR_DIR_RAIZ."img/arrow-right.png'/> Docteur:</b> ".$medico;
	$html .= "<br><br><br><br><br><br>".$espacos."<b style='text-decoration:underline'>Documents soumis:</b>";
	$html .= "<br><br>".$espacos."(&nbsp&nbsp&nbsp&nbsp) Document Personnel Avec Photo";
	$html .= "<br><br>".$espacos."(&nbsp&nbsp&nbsp&nbsp) Adresse Du Composant";
	$html .= "<br><br>".$espacos."(&nbsp&nbsp&nbsp&nbsp) Certificat D'Auto-École";
	$html .= "<br><br>".$espacos."(&nbsp&nbsp&nbsp&nbsp) Rapport D'Examen Médical";
	$html .= "<br><br>".$espacos."(&nbsp&nbsp&nbsp&nbsp) Qualification Précédente";
	$html .= "<br><br><br><br><br><br><div style='text-align:justify; padding:0 50 0 50;'>Ce document doit être livré avec les documents obligatoires pour l'obtention des Permis de Conduire, au DNTT le plus proche, et la libération pour la délivrance des Permis de Conduire interviendra après confirmation de réception par DNTT, via le Système SIGPC. </div>";
	$html .= "<br><br><div style='text-align:center;'><img src='".VAR_DIR_FILES."files/temp/".$qrcode_img."'/></div>";
	$html .= "</span>";
	
	return $html; 	
}

function verificarAutorizacaoProcesso($processo_id) {
	$valida = 1;
	$linha = fSQL::SQL_SELECT_ONE("status,cancelamento_suspensao_id,suspensao_i,suspensao_f","axl_processo","id = '".$processo_id."'");
	if($linha["status"] >= "3"){//processo já em impressão/cancelado não prossegue
		$valida = 0;
	}elseif($linha["suspensao_i"] <= time() and $linha["suspensao_f"] >= time()){
		$valida = 0;
	}elseif($linha["cancelamento_suspensao_id"] >= "1"){
		//solicitação em aberto - não pode autorizar emissão nem cadac (aguardar)
		$cont = fSQL::SQL_CONTAGEM("axl_cancelamento_suspensao","id = '".$linha["cancelamento_suspensao_id"]."' AND status = '0'");
		if($cont >= "1"){
			$valida_ws = 0;
		}
	}	
