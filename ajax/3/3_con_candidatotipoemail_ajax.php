...
if($ajax == "visualizar"){
	...
	$tabela_lerLog = "cad_candidato_email_tipo";
...
if($id_a != "0"){
	$campos = "*";
	$resu1 = fSQL::SQL_SELECT_DUPLO($campos, "cad_candidato_email_tipo", "id = '$id_a'", "");
...
if($cont_encontrou == "0"){
	...
	//contgem
	$contTotal = fSQL::SQL_CONTAGEM("cad_candidato_fisico_email", "tipo_id = '$id_a'")+fSQL::SQL_CONTAGEM("cad_candidato_juridico_email", "tipo_id = '$id_a'");
...
$(document).ready(function(){
	...
	$('#div_displayTitulo<?=$INC_FAISHER["div"]?>').html('VISUALIZAR DADOS DO TIPO DE E-MAIL #<?=$id_a?>');
							...
							<div class="accordion accordion-widget" id="ac_<?=$boxUI_id?>">
								<div class="accordion-group" style="margin-bottom:0;">
									...
									<div id="acc_<?=$boxUI_id?>" class="accordion-body collapse <?php if(($boxUI_status == "1") or ($boxUI_status == "off")){?> in" style="height: auto;"<?php }else{ echo '" style="height:0px;"';}?>>
										...
										<?php if($contTotal >= "1"){ $cont_exib++; $d["content"] = "QTD Emails[,]".$contTotal; $d["type"] = "text"; $PRINT_ARRAY[] = $d; ?>
										<div class="control-group">
											<label class="control-label">QTD Emails</label>
											<div class="controls display">
											  <?=$contTotal?>
											</div>
										</div>
...
?>  
  ...
  <input name="nome" id="nome" type="hidden" value="cadastro_tiposemail_<?=$id_a?>-<?=date('d-m-Y')?>" />
  <input name="titulo" id="titulo" type="hidden" value="Tipos de E-mail" />
...
if($id_a != "0"){
	$cont = "0";
	$campos = "*";
	$resu1 = fSQL::SQL_SELECT_SIMPLES($campos, "cad_candidato_email_tipo", "id = '$id_a'", "");
	while($linha1 = fSQL::FETCH_ASSOC($resu1)){
	$id_a = $linha1["id"];
	$legenda_a = $linha1["legenda"];
	$user_a = $linha1["user"];//quem realizou o cadastro
...
<?php }else{ //if(isset($_GET["POP"])){ ?>
	$(document).ready(function(){
		...
		$('#c_id<?=$INC_FAISHER["div"]?>').val('<?=$id_a?>');//alimenta id de abertura
		<?php if($id_a == "0"){ ?>
		$('#div_displayTitulo<?=$INC_FAISHER["div"]?>').html('CADASTRAR NOVO TIPO DE E-MAIL');
		$('#bt_edit<?=$INC_FAISHER["div"]?>, #bt_visual<?=$INC_FAISHER["div"]?>').hide();
		<?php }else{ ?>
		$('#div_displayTitulo<?=$INC_FAISHER["div"]?>').html('EDITAR CADASTRO TIPO DE E-MAIL #<?=$id_a?>');
...
if(isset($_GET["id_excluir"])){
	...
	if($cVLogin->loginAcesso($INC_FAISHER["permissao"],"exc")){
		...
		$conta_ver = fSQL::SQL_CONTAGEM("cad_candidato_fisico_email", "tipo_id = '$id_excluir'","1")+fSQL::SQL_CONTAGEM("cad_candidato_juridico_email", "tipo_id = '$id_excluir'","1");
		//verifica se existe registros vinculados
		if($conta_ver == "0"){//verifica permissão	
			//exclue o registro
			$tabela = "cad_candidato_email_tipo";
			$condicao = "id = '$id_excluir'";
			$resu2 =  fSQL::SQL_DELETE_SIMPLES($tabela, $condicao);
			fGERAL::delRegistroLog("cad_candidato_email_tipo",$id_excluir,$cVLogin->userReg());//salvar/gerar log de exclusao
				
			//GERA AÇÃO DO USUÁRIO NA TABELA
			$cVLogin->addAcaoUser("cad_candidato_email_tipo", "excluir", $id_excluir);//addAcaoUser($TABELA, $ACAO, $REGISTRO)
			//ADICIONA MENSAGEM > SUCESSO ALERTA ERRO INFO
...
//VALIDAÇÔES ------------------------------**********
	...
	$cont_ = "0";
	$resu1 = fSQL::SQL_SELECT_SIMPLES("id", "cad_candidato_email_tipo", "legenda = '$legenda_a' $sql_complemto", "");
	while($linha1 = fSQL::FETCH_ASSOC($resu1)){
		$cont_++;
	}//fim while
...
if($id_a == "0"){ //############# IF - GRAVA NOVO REGISTRO |-----> SQL CADASTRO
	...
	//GERA AÇÃO DO USUÁRIO NA TABELA
	$cVLogin->addAcaoUser("cad_candidato_email_tipo", "adicionar", $id_a);//addAcaoUser($TABELA, $ACAO, $REGISTRO)
	//GERA LOGS DE TABELAS
	if(isset($ARRAY_LOG)){//gerar log de auditoria	
		fGERAL::gravaLog("cad_candidato_email_tipo",$id_a,$ARRAY_LOG,$cVLogin->userReg());
	}//if(isset($ARRAY_LOG)){
	//ADICIONA MENSAGEM > SUCESSO ALERTA ERRO INFO
	$cMSG->addMSG("SUCESSO","Tipo de email cadastrado com sucesso!$msg_cont <a href=\"#\" onclick=\"janelaAcao".$INC_FAISHER["div"]."(\\'visualizar\\',\\'id_a=".$id_a."\\');return false;\" class=\"btn sAcao\" rel=\"tooltip\" title=\"Visualizar\"><i class=\"icon-search\"></i></a>");
	...
	//GERA AÇÃO DO USUÁRIO NA TABELA
	$cVLogin->addAcaoUser("cad_candidato_email_tipo", "editar", $id_a);//addAcaoUser($TABELA, $ACAO, $REGISTRO)
	//GERA LOGS DE TABELAS
	if(isset($ARRAY_LOG)){//gerar log de auditoria	
		fGERAL::gravaLog("cad_candidato_email_tipo",$id_a,$ARRAY_LOG,$cVLogin->userReg());
	}//if(isset($ARRAY_LOG)){
...
//vars para consulta no DB referente a paginação abaixo
	...
	$SQL_tabela = "cad_candidato_email_tipo"; //tabela
	$SQL_where = ""; //condição
