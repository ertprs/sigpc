...
if($ajax == "visualizar"){
	...
	$tabela_lerLog = "cad_candidato_juridico_natureza";
...
if($id_a != "0"){
	$campos = "*";
	$resu1 = fSQL::SQL_SELECT_DUPLO($campos, "cad_candidato_juridico_natureza", "id = '$id_a'", "");
	while($linha1 = fSQL::FETCH_ASSOC($resu1)){
	$id_a = $linha1["id"];
	$codigo_a = $linha1["codigo"];
...
$(document).ready(function(){
	...
	$('#div_displayTitulo<?=$INC_FAISHER["div"]?>').html('VISUALIZAR DADOS DO NATUREZA DE CANDIDATO JURÍDICO #<?=$id_a?>');
							...
							<div class="accordion accordion-widget" id="ac_<?=$boxUI_id?>">
								<div class="accordion-group" style="margin-bottom:0;">
									...
									<div id="acc_<?=$boxUI_id?>" class="accordion-body collapse <?php if(($boxUI_status == "1") or ($boxUI_status == "off")){?> in" style="height: auto;"<?php }else{ echo '" style="height:0px;"';}?>>
										...
										<?php $cont_exib++; $d["content"] = "Código[,]".$codigo_a; $d["type"] = "text"; $PRINT_ARRAY[] = $d; ?>
										<div class="control-group">
...
?>  
  ...
  <input name="nome" id="nome" type="hidden" value="cadastro_tiposnaturezacandidatosjuridico_<?=$id_a?>-<?=date('d-m-Y')?>" />
  <input name="titulo" id="titulo" type="hidden" value="Tipos de Natureza Candidato Jurídico" />
...
if($id_a != "0"){
	$cont = "0";
	$campos = "*";
	$resu1 = fSQL::SQL_SELECT_SIMPLES($campos, "cad_candidato_juridico_natureza", "id = '$id_a'", "");
	while($linha1 = fSQL::FETCH_ASSOC($resu1)){
	$id_a = $linha1["id"];
	$codigo_a = $linha1["codigo"];
	$legenda_a = $linha1["legenda"];
...
<?php }else{ //if(isset($_GET["POP"])){ ?>
	$(document).ready(function(){
		...
		$('#c_id<?=$INC_FAISHER["div"]?>').val('<?=$id_a?>');//alimenta id de abertura
		<?php if($id_a == "0"){ ?>
		$('#div_displayTitulo<?=$INC_FAISHER["div"]?>').html('CADASTRAR NOVO NATUREZA DE CANDIDATO JURÍDICO');
		$('#bt_edit<?=$INC_FAISHER["div"]?>, #bt_visual<?=$INC_FAISHER["div"]?>').hide();
		<?php }else{ ?>
		$('#div_displayTitulo<?=$INC_FAISHER["div"]?>').html('EDITAR CADASTRO NATUREZA DE CANDIDATO JURÍDICO #<?=$id_a?>');
		$('#bt_edit<?=$INC_FAISHER["div"]?>').hide();
		<?php }?>
	
	});
	<?php if($id_a == "0"){ ?>
	$.doTimeout('vTimerCursor<?=$INC_FAISHER["div"]?>', 500, function(){ ancoraHtml('#ancAcao<?=$INC_FAISHER["div"]?>'); $('#<?=$formCadastroPincipal?> #codigo').focus(); });//TIMER
	<?php }?>
<?php }//else{ //if(isset($_GET["POP"])){ ?>
								...
								<div class="accordion-group" style="margin-bottom:0;">
									...
									<div id="acc_<?=$boxUI_id?>" class="accordion-body collapse <?php if(($boxUI_status == "1") or ($boxUI_status == "off")){?> in" style="height: auto;"<?php }else{ echo '" style="height:0px;"';}?>>
										...
										<div class="control-group">
											<label class="control-label">Código</label>
											<div class="controls">
											  <input type="text" name="codigo" id="codigo" value="<?=$codigo_a?>" class="input-xlarge span9" data-rule-required="true">
											</div>
										</div>
										<div class="control-group">
											<label class="control-label">Legenda/nome</label>
...
if(isset($_GET["id_excluir"])){
	...
	if($cVLogin->loginAcesso($INC_FAISHER["permissao"],"exc")){
		...
		$conta_ver = fSQL::SQL_CONTAGEM("cad_candidato_juridico", "natureza_id = '$id_excluir'","1");
		...
		if($conta_ver == "0"){//verifica permissão
			...
			fGERAL::delRegistroLog("cad_candidato_juridico_natureza",$id_excluir,$cVLogin->userReg());//salvar/gerar log de exclusao
				
			//GERA AÇÃO DO USUÁRIO NA TABELA
			$cVLogin->addAcaoUser("cad_candidato_juridico_natureza", "excluir", $id_excluir);//addAcaoUser($TABELA, $ACAO, $REGISTRO)
			//ADICIONA MENSAGEM > SUCESSO ALERTA ERRO INFO
...
if(isset($_POST["id_a"])){
	...
	//recebe vars - geral
	$codigo_a = getpost_sql($_POST["codigo"]);
...
//VALIDAÇÔES ------------------------------**********
	...
	$cont_ = "0";
	$resu1 = fSQL::SQL_SELECT_SIMPLES("id", "cad_candidato_juridico_natureza", "legenda = '$legenda_a' $sql_complemto", "");
	while($linha1 = fSQL::FETCH_ASSOC($resu1)){
		$cont_++;
	}//fim while
	...
	$cont_ = "0";
	$resu1 = fSQL::SQL_SELECT_SIMPLES("id", "cad_candidato_juridico_natureza", "codigo = '$codigo_a' $sql_complemto", "");
	while($linha1 = fSQL::FETCH_ASSOC($resu1)){
		$cont_++;
	}//fim while
	if($cont_ >= "1"){ if($verifica_erro != "0"){ $verifica_erro .= "<br>"; }else{ $verifica_erro = ""; }
		$verifica_erro .= "- O código ($codigo_a) está em uso, não pode ser utilizados código iguais! Utilize outro nome.";//msg
	}//fim if valida campo
...
if($id_a == "0"){ //############# IF - GRAVA NOVO REGISTRO |-----> SQL CADASTRO
	...
	$campos = "id,codigo,legenda,user,time,user_a,sync";
	$valores = array($id_a,$codigo_a,$legenda_a,$cVLogin->userReg(),time(),"0",time());
	...
	//GERA AÇÃO DO USUÁRIO NA TABELA
	$cVLogin->addAcaoUser("cad_candidato_juridico_natureza", "adicionar", $id_a);//addAcaoUser($TABELA, $ACAO, $REGISTRO)
	//GERA LOGS DE TABELAS
	if(isset($ARRAY_LOG)){//gerar log de auditoria	
		fGERAL::gravaLog("cad_candidato_juridico_natureza",$id_a,$ARRAY_LOG,$cVLogin->userReg());
	}//if(isset($ARRAY_LOG)){
	//ADICIONA MENSAGEM > SUCESSO ALERTA ERRO INFO
	$cMSG->addMSG("SUCESSO","Tipo de natureza jurídica cadastrado com sucesso!$msg_cont <a href=\"#\" onclick=\"janelaAcao".$INC_FAISHER["div"]."(\\'visualizar\\',\\'id_a=".$id_a."\\');return false;\" class=\"btn sAcao\" rel=\"tooltip\" title=\"Visualizar\"><i class=\"icon-search\"></i></a>");
	...
	$campos = "codigo,legenda,user_a,sync";
	$tabela = "cad_candidato_juridico_natureza";
	$valores = array($codigo_a,$legenda_a,$cVLogin->userReg(),time());
	$condicao = "id='$id_a'";
	...
	//GERA AÇÃO DO USUÁRIO NA TABELA
	$cVLogin->addAcaoUser("cad_candidato_juridico_natureza", "editar", $id_a);//addAcaoUser($TABELA, $ACAO, $REGISTRO)
	//GERA LOGS DE TABELAS
	if(isset($ARRAY_LOG)){//gerar log de auditoria	
		fGERAL::gravaLog("cad_candidato_juridico_natureza",$id_a,$ARRAY_LOG,$cVLogin->userReg());
	}//if(isset($ARRAY_LOG)){
...
//vars para consulta no DB referente a paginação abaixo
	$SQL_campos = "id,codigo,legenda,sync"; //campos da tabela
	$SQL_tabela = "cad_candidato_juridico_natureza"; //tabela
	$SQL_where = ""; //condição
...
if($rapida_b != ""){  $filtro_marca[] = $rapida_b;
		...
		$SQL_where .= " ( `id` = '$rapida_b' OR `codigo` LIKE '%$rapida_b%' OR `legenda` LIKE '%$rapida_b%' ) "; //condição 
	...
	<table id="datatable_principal<?=$INC_FAISHER["div"]?>" class="table table-hover table-nomargin table-bordered dataTable">
		<thead>
			<tr>
				...
				<?php $c_or = "codigo"; ?><th class="<?=css_ordem($c_or,$ORDEM_C,$ORDEM)?>" id="<?=$c_or?>&ORDEM=<?=alterna_ordem($ORDEM)?>">Código</th>
				<?php $c_or = "legenda"; ?><th class="<?=css_ordem($c_or,$ORDEM_C,$ORDEM)?>" id="<?=$c_or?>&ORDEM=<?=alterna_ordem($ORDEM)?>">Legenda/Nome</th>
				<th>Nº Candidato(s)</th>
...
while($linha1 = fSQL::FETCH_ASSOC($QueryListaPag)){
	...
	//contgem
	$contTotal = fSQL::SQL_CONTAGEM("cad_candidato_juridico", "natureza_id = '$id_a'");

?>
										<tr>
											<td class="sVisu"><b>#<?=$id_a?></b></td>
											<td class="sVisu"><b>#<?=$codigo?></b></td>
