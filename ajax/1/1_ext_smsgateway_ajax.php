...
if($ajax == "geraChave"){
	...
	$var = fGERAL::gerarChave(32,"nivel2");
	echo "<script>$.doTimeout('vTimerLoad".$INC_FAISHER["div"]."', 100, function(){ $('#".$formCadastroPincipal." #acesso_chave').val('".$var."'); });</script>";
}//fim ajax -------------------<<<<
...
if($ajax == "geraSenha"){
	$formCadastroPincipal = $_GET["formCadastroPincipal"];
	$var = fGERAL::gerarChave(24,"nivel2");
...
if($ajax == "visualizar"){
	...
	$tabela_lerLog = "sys_webservice_sms";
...
if($id_a != "0"){
	$campos = "*";
	$resu1 = fSQL::SQL_SELECT_SIMPLES($campos, "sys_webservice_sms", "id = '$id_a'", "");
...
if($cont_encontrou == "0"){
	...
	$acao_a = legAcaoWebservice("sys_webservice_sms");
	
	$resu1 = fSQL::SQL_SELECT_SIMPLES("id,acesso_login,acesso_senha,acesso_chave,acesso_ip,acesso_metodos,t_sync,t_get,t_send,time_expira,status", "sys_webservice_fws", "acao = '$acao_a' AND acao_id = '$id_a'", "");
	while($linha1 = fSQL::FETCH_ASSOC($resu1)){
	$id_i = $linha1["id"];
	$acesso_login_i = $linha1["acesso_login"];
	$acesso_senha_i = $linha1["acesso_senha"];
	$acesso_chave_i = $linha1["acesso_chave"];
	$acesso_ip_i = $linha1["acesso_ip"];
	$acesso_metodos_i = $linha1["acesso_metodos"];
	$t_sync_i = $linha1["t_sync"];
	$t_get_i = $linha1["t_get"];
	$t_send_i = $linha1["t_send"];
...
$(document).ready(function(){
	...
	$('#div_displayTitulo<?=$INC_FAISHER["div"]?>').html('VISUALIZAR DADOS DE WS SMS #<?=$id_a?>');
...
<form nome="<?=$formVisualizaPincipal?>" id="<?=$formVisualizaPincipal?>" method="post" class="form-horizontal form-column form-bordered form-validate" action="export.php" target="_blank">
             <div style="padding-top:1px;" id="formVisualizarMSG<?=$INC_FAISHER["div"]?>"></div>
						...
						$boxUI_titulo = "Dados Gerais (Informações para o Gestor TI)";// titulo
...
?>    
						...
						$boxUI_titulo = "Dados do Web Service fWS (uso do cliente)";// titulo
						$boxUI_status = "off";// 1 - aberto, 0 - fechado, off - desligado
						$boxUI_rodape = "- ".$boxUI_titulo;//texto rodape
						$cont_exib = "0";//contador para exibicao dos dados
						?>
							<div class="accordion accordion-widget" id="ac_<?=$boxUI_id?>">
								<div class="accordion-group" style="margin-bottom:0;">
									...
									<div id="acc_<?=$boxUI_id?>" class="accordion-body collapse <?php if(($boxUI_status == "1") or ($boxUI_status == "off")){?> in" style="height: auto;"<?php }else{ echo '" style="height:0px;"';}?>>
										...
										<?php $cont_exib++; $d["content"] = "URL conexão (WEB SERVICE)[,]".SYS_URLRAIZ."webservice/sms/"; $d["type"] = "text"; $PRINT_ARRAY[] = $d; ?>
										<div class="control-group">
											<label class="control-label">URL conexão (WEB SERVICE)</label>
											<div class="controls display-plus" id="url<?=$INC_FAISHER["div"]?>" onClick="SelectText('url<?=$INC_FAISHER["div"]?>');">
											  <?=SYS_URLRAIZ?>webservice/sms/
											</div>
										</div>
										<?php $cont_exib++; $d["content"] = "URL GET (funções auxiliares)[,]".SYS_URLRAIZ."get/"; $d["type"] = "text"; $PRINT_ARRAY[] = $d; ?>
										<div class="control-group">
											<label class="control-label">URL GET (funções auxiliares)</label>
											<div class="controls display-plus" id="urlg<?=$INC_FAISHER["div"]?>" onClick="SelectText('urlg<?=$INC_FAISHER["div"]?>');">
											  <?=SYS_URLRAIZ?>get/
											</div>
										</div>
										<?php $cont_exib++; $d["content"] = "fWS_ID[,]".$id_i; $d["type"] = "text"; $PRINT_ARRAY[] = $d; ?>
										<div class="control-group">
											<label class="control-label">fWS_ID</label>
											<div class="controls display-plus" id="idacess<?=$INC_FAISHER["div"]?>" onClick="SelectText('idacess<?=$INC_FAISHER["div"]?>');">
											  <?=$id_i?>
											</div>
										</div>
										<?php $cont_exib++; $d["content"] = "Login de acesso[,]".$acesso_login_i; $d["type"] = "text"; $PRINT_ARRAY[] = $d; ?>
										<div class="control-group">
											<label class="control-label">Login de acesso</label>
											<div class="controls display-plus" id="logacess<?=$INC_FAISHER["div"]?>" onClick="SelectText('logacess<?=$INC_FAISHER["div"]?>');">
											  <?=$acesso_login_i?>
											</div>
										</div>
										...
										<div class="control-group">
											<label class="control-label">Chave de acesso (fCriptoJs)</label>
											<div class="controls display-plus" id="chacess<?=$INC_FAISHER["div"]?>" onClick="SelectText('chacess<?=$INC_FAISHER["div"]?>');">
											  <?=$acesso_chave_i?>
											</div>
										</div>
										<?php if($ip_cliente_i != ""){ $cont_exib++; $d["content"] = "IP do cliente[,]".$acesso_ip_i; $d["type"] = "text"; $PRINT_ARRAY[] = $d; ?>
										<div class="control-group">
...
?>      
                    	...
                    	<?php // BLOCO DE DADOS -----------------------accordion-widget--------------------------- >>>
                        $boxUI_id = "daddoswebsermtd".$array_temp;//id de controle
						$boxUI_titulo = "Métodos Web Service fWS (autorização)";// titulo
						$boxUI_status = "off";// 1 - aberto, 0 - fechado, off - desligado
						$boxUI_rodape = "- ".$boxUI_titulo;//texto rodape
						$cont_exib = "0";//contador para exibicao dos dados
						?>
							<div class="accordion accordion-widget" id="ac_<?=$boxUI_id?>">
								<div class="accordion-group" style="margin-bottom:0;">
									...
									<div id="acc_<?=$boxUI_id?>" class="accordion-body collapse <?php if(($boxUI_status == "1") or ($boxUI_status == "off")){?> in" style="height: auto;"<?php }else{ echo '" style="height:0px;"';}?>>
										<div class="accordion-inner" style="padding:0;">
										<?php $d["content"] = "Ajuda[,]ESSES SÃO OS MÉTODOS DISPONÍVEIS A ESSA CONEXÃO"; $d["type"] = "text"; $PRINT_ARRAY[] = $d; ?>
										<div class="control-group">
											<label class="control-label span3">Ajuda</label>
											<div class="controls display-plus span9">
											  ESSES SÃO OS MÉTODOS DISPONÍVEIS A ESSA CONEXÃO
											</div>
										</div>
										<?php $metd = "syncSMS"; $metdLeg = "Busca/envia mansagens SMS"; ?>
										<?php if(preg_match("/\.".$metd."\./i", $acesso_metodos_i)){ $cont_exib++; $d["content"] = $metd."[,]".$metdLeg; $d["type"] = "text"; $PRINT_ARRAY[] = $d; ?>
										<div class="control-group">
											<label class="control-label span3"><?=$metd?></label>
											<div class="controls display-plus span9">
											  <?=$metdLeg?>
											</div>
										</div>
                                        <?php }?>
										<?php $metd = "confirmaEnvioSMS"; $metdLeg = "Confirma envio de SMS"; ?>
										<?php if(preg_match("/\.".$metd."\./i", $acesso_metodos_i)){ $cont_exib++; $d["content"] = $metd."[,]".$metdLeg; $d["type"] = "text"; $PRINT_ARRAY[] = $d; ?>
										<div class="control-group">
											<label class="control-label span3"><?=$metd?></label>
											<div class="controls display-plus span9">
...
?>  
  ...
  <input name="nome" id="nome" type="hidden" value="cadastro_wssms_<?=$id_a?>-<?=date('d-m-Y')?>" />
  <input name="titulo" id="titulo" type="hidden" value="Web Service SMS (não divulgue esses dados)" />
...
if($id_a == "0"){
	...
	$numero_a = "";
	$acesso_login_i = "";
	$acesso_senha_i = fGERAL::gerarChave(24,"nivel2");
	$acesso_chave_i = fGERAL::gerarChave(32,"nivel2");
	$acesso_ip_i = "";
...
<?php }else{ //if(isset($_GET["POP"])){ ?>
	$(document).ready(function(){
		...
		$('#c_id<?=$INC_FAISHER["div"]?>').val('<?=$id_a?>');//alimenta id de abertura
		<?php if($id_a == "0"){ ?>
		$('#div_displayTitulo<?=$INC_FAISHER["div"]?>').html('CADASTRAR NOVO WS UPDATE');
		$('#bt_edit<?=$INC_FAISHER["div"]?>, #bt_visual<?=$INC_FAISHER["div"]?>').hide();
		<?php }else{ ?>
		$('#div_displayTitulo<?=$INC_FAISHER["div"]?>').html("EDITAR CADASTRO DE WS UPDATE #<?=$id_a?>");
									...
									<div id="acc_<?=$boxUI_id?>" class="accordion-body collapse <?php if(($boxUI_status == "1") or ($boxUI_status == "off")){?> in" style="height: auto;"<?php }else{ echo '" style="height:0px;"';}?>>
										...
										<div class="control-group">
											...
											<div class="controls">
											  ...
											  <span class="help-block"><i class="icon-info-sign"></i> Endereço/referência de publicação do uso da conexão de web service.</span>
											</div>
										</div>
										<div class="control-group">
											<label class="control-label">Nome do responsável técnico</label>
							...
							<div class="accordion accordion-widget" id="ac_<?=$boxUI_id?>">
								<div class="accordion-group" style="margin-bottom:0;">
									...
									<div id="acc_<?=$boxUI_id?>" class="accordion-body collapse <?php if(($boxUI_status == "1") or ($boxUI_status == "off")){?> in" style="height: auto;"<?php }else{ echo '" style="height:0px;"';}?>>
										...
										<div class="control-group">
											<label class="control-label">URL conexão (WEB SERVICE)</label>
											<div class="controls display-plus" id="selurl<?=$INC_FAISHER["div"]?>" onClick="SelectText('selurl<?=$INC_FAISHER["div"]?>');">
											  <?=SYS_URLRAIZ?>webservice/sms/
											</div>
										</div>
										<div class="control-group">
											<label class="control-label">URL GET (funções auxiliares)</label>
											<div class="controls display-plus" id="selurlg<?=$INC_FAISHER["div"]?>" onClick="SelectText('selurlg<?=$INC_FAISHER["div"]?>');">
											  <?=SYS_URLRAIZ?>get/
											</div>
										</div>
										<div class="control-group">
											<label class="control-label">fWS_ID</label>
											<div class="controls display-plus">
											  <?php if($id_a >= "1"){ echo $id_i; }else{ echo "É GERADO APÓS O CADASTRO INICIAL (utilize visualizar)"; }?>
											</div>
										</div>
										<div class="control-group">
											<label class="control-label">Login de acesso</label>
											<div class="controls">
											  <input type="text" name="acesso_login" id="acesso_login" value="<?=$acesso_login_i?>" class="span6" data-rule-required="true">
											</div>
										</div>
										<div class="control-group">
											<label class="control-label">Senha de acesso</label>
											<div class="controls">
											  <input type="text" name="acesso_senha" id="acesso_senha" value="<?=$acesso_senha_i?>" class="span6" data-rule-required="true">
										...
										<div class="control-group">
											<label class="control-label">Chave de acesso (fCriptoJs)</label>
											<div class="controls">
											  <input type="text" name="acesso_chave" id="acesso_chave" value="<?=$acesso_chave_i?>" class="span6" readonly data-rule-required="true">
                                              <button class="btn" onclick="geraChave<?=$INC_FAISHER["div"]?>();return false;" rel="tooltip" data-placement="left" data-original-title="Gerar/alterar senha de acesso"><i class="icon-random"></i> <?php if($id_a >= "1"){ echo 'Alterar chave de acesso';  }else{ echo 'Gerar nova chave de acesso'; }?></button>
											</div>
										</div>
<script>
function geraSenha<?=$INC_FAISHER["div"]?>(){
	if(confirm("Realmente confirma a alteração da SENHA DE ACESSO? <?php if($id_a >= "1"){ echo '\\n\\n\\nCaso exista conexão com outros serviços, deverá atualizar a senha nos mesmos!';  }?>") == true){
		faisher_ajax('div_oculta', '', '<?=$AJAX_PAG?>', 'faisher=<?=$faisher?>&formCadastroPincipal=<?=$formCadastroPincipal?>&ajax=geraSenha', 'get', 'ADD');
	}
}//geraSenha

function geraChave<?=$INC_FAISHER["div"]?>(){
	if(confirm("Realmente confirma a alteração da CHAVE DE ACESSO? <?php if($id_a >= "1"){ echo '\\n\\n\\nCaso exista conexão com outros serviços, deverá atualizar as chaves nos mesmos!';  }?>") == true){
		faisher_ajax('div_oculta', '', '<?=$AJAX_PAG?>', 'faisher=<?=$faisher?>&formCadastroPincipal=<?=$formCadastroPincipal?>&ajax=geraChave', 'get', 'ADD');
	}
}//geraChave
</script>
										...
										<div class="control-group">
											<label class="control-label">IP do cliente</label>
											<div class="controls">
											  <input type="text" name="acesso_ip" id="acesso_ip" value="<?=$acesso_ip_i?>" placeholder="Sem IP de Cliente Definido" class="span4">
                    	...
                    	<?php // BLOCO DE DADOS -----------------------accordion-widget--------------------------- >>>
                        $boxUI_id = "dadoswebservicemdt".$array_temp;//id de controle
						$boxUI_titulo = "Métodos Web Service fWS (autorização)";// titulo
						$boxUI_status = "off";// 1 - aberto, 0 - fechado, off - desligado
						$boxUI_rodape = "- ".$boxUI_titulo;//texto rodape
						?>
								...
								<div class="accordion-group" style="margin-bottom:0;">
									...
									<div id="acc_<?=$boxUI_id?>" class="accordion-body collapse <?php if(($boxUI_status == "1") or ($boxUI_status == "off")){?> in" style="height: auto;"<?php }else{ echo '" style="height:0px;"';}?>>
										...
										<div class="control-group">
											<label class="control-label span3">Ajuda</label>
											<div class="controls display-plus span9">
											  <B>*SÓ SERÁ POSSÍVEL UTILIZAR OS MÉTODOS SELECIONADOS!</B>
											</div>
										</div>
										...
										<?php $metd = "syncSMS"; $metdLeg = "Busca/envia mansagens SMS"; ?>
											...
											<div class="controls span9">
												<div class="check-line">
													<input type="checkbox" id="<?=$metd?>" name="<?=$metd?>" <?php if(preg_match("/\.".$metd."\./i", $acesso_metodos_i)){ echo 'checked="checked"'; }?> class='<?=$INC_FAISHER["div"]?>-icheck' data-skin="square" data-color="green"> <label class='inline' for="<?=$metd?>">ATIVAR: <B><?=$metdLeg?></B></label>
												</div>
											</div>
										</div>
										<div class="control-group">
...
if(isset($_GET["id_excluir"])){
	...
	if($cVLogin->loginAcesso($INC_FAISHER["permissao"],"exc")){
		$resu1 = fSQL::SQL_SELECT_SIMPLES("id,acao", "sys_webservice_sms", "id = '$id_excluir'", "");
		while($linha1 = fSQL::FETCH_ASSOC($resu1)){
			$id_ = $linha1["id"];
			$acao_ = $linha1["acao"];
			
			$resu2 = fSQL::SQL_SELECT_SIMPLES("id", "sys_webservice_fws", "acao = '$acao_' AND acao_id = '$id_'", "");
			while($linha2 = fSQL::FETCH_ASSOC($resu2)){
				...
				fGERAL::delRegistroLog("sys_webservice_fws",$id_i,$cVLogin->userReg());//salvar/gerar log de exclusao
			}//fim while
			
			//exclue o registro
			$tabela = "sys_webservice_sms";
			$condicao = "id = '$id_'";
			fSQL::SQL_DELETE_SIMPLES($tabela, $condicao);
			fGERAL::delRegistroLog("sys_webservice_sms",$id_,$cVLogin->userReg());//salvar/gerar log de exclusao
			//GERA AÇÃO DO USUÁRIO NA TABELA
			$cVLogin->addAcaoUser("sys_webservice_fws", "excluir", $id_excluir);//addAcaoUser($TABELA, $ACAO, $REGISTRO)
			//ADICIONA MENSAGEM > SUCESSO ALERTA ERRO INFO
...
if(isset($_POST["id_a"])){
	...
	$acesso_login_a = getpost_sql($_POST["acesso_login"]);
	$acesso_senha_a = getpost_sql($_POST["acesso_senha"]);
	$acesso_chave_a = getpost_sql($_POST["acesso_chave"]);
	$acesso_ip_a = getpost_sql($_POST["acesso_ip"], "NULL");
	$time_expira_a = getpost_sql($_POST["time_expira"]);
	$status_a = getpost_sql($_POST["status"]);
	$acao_a = legAcaoWebservice("sys_webservice_sms");
	$acesso_metodos_a = "";	
	$varM = "syncSMS"; if(isset($_POST["$varM"])){ $acesso_metodos_a .= ".".$varM;	}
	$varM = "confirmaEnvioSMS"; if(isset($_POST["$varM"])){ $acesso_metodos_a .= ".".$varM;	}
	if($acesso_metodos_a != ""){ $acesso_metodos_a = "[".$acesso_metodos_a.".]"; }
...
//VALIDAÇÔES ------------------------------**********
	...
	$cont_ = "0";
	$resu1 = fSQL::SQL_SELECT_SIMPLES("id", "sys_webservice_sms", "legenda = '$legenda_a' $sql_complemto", "");
	while($linha1 = fSQL::FETCH_ASSOC($resu1)){
		$cont_++;
	}//fim while
	...
	//verifica se já existe no sistem
	$sql_complemto = " AND acao = '$acao_a' AND acao_id != '$id_a'";	
	if($id_a == "0"){ $sql_complemto = ""; }//if($id_a == "0"){
	$cont_ = "0";
	$resu1 = fSQL::SQL_SELECT_SIMPLES("id", "sys_webservice_fws", "acesso_login = '$acesso_login_a' AND acesso_senha = '$acesso_senha_a' $sql_complemto", "");
	while($linha1 = fSQL::FETCH_ASSOC($resu1)){
		$cont_++;
	}//fim while
	if($cont_ >= "1"){ if($verifica_erro != "0"){ $verifica_erro .= "<br>"; }else{ $verifica_erro = ""; }
		$verifica_erro .= "- O Login/senha de acesso gerado está em uso por outro web service! Utilize o botão Gerar/Alterar para modificar.";//msg
	}//fim if valida campo

	//valida campo acesso_chave_a -- XXX
	if(strlen($acesso_chave_a) != "32"){ if($verifica_erro != "0"){ $verifica_erro .= "<br>"; }else{ $verifica_erro = ""; }
		$verifica_erro .= "- A chave informada não tem 32 caracteres, verefique!";//msg
	}//fim if valida campo
	...
	//valida campo acesso_metodos_a -- XXX
	if($acesso_metodos_a == ""){ if($verifica_erro != "0"){ $verifica_erro .= "<br>"; }else{ $verifica_erro = ""; }
		$verifica_erro .= "- Não existe método selecionado! Caso queira desativar o acesso, utilize o status!";//msg
	}//fim if valida campo
...
if($id_a == "0"){ //############# IF - GRAVA NOVO REGISTRO |-----> SQL CADASTRO
	...
	$campos = "id,acao,legenda,url,responsavel,fone,user,time,user_a,sync";
	$valores = array($id_a,$acao_a,$legenda_a,$url_a,$responsavel_a,$fone_a,$cVLogin->userReg(),time(),"0",time());
	...
	$campos = "id,acao,acao_id,referencia,acesso_login,acesso_senha,acesso_chave,acesso_ip,acesso_metodos,time_expira,status";
	$valores = array($id_i,$acao_a,$id_a,$legenda_a,$acesso_login_a,$acesso_senha_a,$acesso_chave_a,$acesso_ip_a,$acesso_metodos_a,$time_expira_a,$status_a);
	fSQL::SQL_INSERT_SIMPLES($campos, $tabela, $valores);
	$ARRAY_LOG[] = fGERAL::arrayLog($tabela,$campos,$valores);//gerar array log de auditoria
	...
	//GERA AÇÃO DO USUÁRIO NA TABELA
	$cVLogin->addAcaoUser("sys_webservice_fws", "adicionar", $id_a);//addAcaoUser($TABELA, $ACAO, $REGISTRO)
	//GERA LOGS DE TABELAS
	if(isset($ARRAY_LOG)){//gerar log de auditoria	
		fGERAL::gravaLog("sys_webservice_sms",$id_a,$ARRAY_LOG,$cVLogin->userReg());
	}//if(isset($ARRAY_LOG)){
	//ADICIONA MENSAGEM > SUCESSO ALERTA ERRO INFO
	...
	//atualiza dados da tabela no DB
	$campos = "referencia,acesso_login,acesso_senha,acesso_chave,acesso_ip,acesso_metodos,time_expira,status";
	$tabela = "sys_webservice_fws";
	$valores = array($legenda_a,$acesso_login_a,$acesso_senha_a,$acesso_chave_a,$acesso_ip_a,$acesso_metodos_a,$time_expira_a,$status_a);
	$condicao = "acao = '$acao_a' AND acao_id = '$id_a'";
	fSQL::SQL_UPDATE_SIMPLES($campos, $tabela, $valores, $condicao);
	$ARRAY_LOG[] = fGERAL::arrayLog($tabela,$campos,$valores);//gerar array log de auditoria
	...
	//GERA AÇÃO DO USUÁRIO NA TABELA
	$cVLogin->addAcaoUser("sys_webservice_fws", "editar", $id_a);//addAcaoUser($TABELA, $ACAO, $REGISTRO)
	//GERA LOGS DE TABELAS
	if(isset($ARRAY_LOG)){//gerar log de auditoria	
		fGERAL::gravaLog("sys_webservice_sms",$id_a,$ARRAY_LOG,$cVLogin->userReg());
	}//if(isset($ARRAY_LOG)){
...
//vars para consulta no DB referente a paginação abaixo
	$SQL_campos = "G.id,G.legenda,G.responsavel,W.t_sync,W.t_get,W.t_send,W.time_expira,W.status"; //campos da tabela
	$SQL_tabela = "sys_webservice_sms G, sys_webservice_fws W"; //tabela
	$SQL_where = "G.acao = W.acao AND G.id = W.acao_id"; //condição
...
while($linha1 = fSQL::FETCH_ASSOC($QueryListaPag)){
	...
	$responsavel = $linha1["responsavel"];
	$t_sync = $linha1["t_sync"];
	$t_get = $linha1["t_get"];
	$t_send = $linha1["t_send"];
...
?>
										<tr>
											<td class="sVisu"><b><?=maiusculo($legenda)?></b><br><i>Resp. <?=$responsavel?></i></td>
											<td class="sVisu">
		<div><button class="btn btn-<?=legCorSyncWebservice($t_sync)?>" style="width:100%;" rel="tooltip" data-placement="left" data-original-title="Comunicação com o cliente">SYNC: <?=difHoraTime($t_sync)?></button></div>
		<div><button class="btn btn-<?=legCorSyncWebservice($t_get)?>" style="margin-top:2px; width:100%;" rel="tooltip" data-placement="left" data-original-title="Copiou dados">GET: <?=difHoraTime($t_get)?></button></div>
		<div><button class="btn btn-<?=legCorSyncWebservice($t_send)?>" style="margin-top:2px; width:100%;" rel="tooltip" data-placement="left" data-original-title="Enviou dados">SEND: <?=difHoraTime($t_send)?></button></div>
                                            </td>
											<td class='sVisu'><?=$status_leg?><br><i>Expirar <?=difHoraTime(time(),$time_expira,"2","")?></i></td>
											<td>
