...
function __autoload($className) {
    ...
    if (isset($_SESSION[MODULO][AXL::K_CURRENT_MODULE])) {
    	$mod_dir = $_SESSION[MODULO][AXL::K_CURRENT_MODULE]->get_dir();
...
class AXL {	
	...
	const NAME = "AXL Livre";
	const VERSION = "1.0.3";
	
	// SESSION KEYS
    const K_CURRENT_MODULE = "AXL_CURRENT_MODULE";
    const K_CURRENT_USER   = "AXL_CURRENT_USER";
    ...
    public static function _include($param) {
        ...
        if (file_exists($url)) {
	        ...
	        // Checa se o arquivo a ser incluso esta na pasta(ou em uma subpasta) do axl
	...
	public static function import($param) { return AXL::_include($param); }
	...
	public static function force_unidade() {
        header("Location:./?" . AXL::K_NEED_UNIDADE);
    }
    ...
    public static function check_access($mod_key) {
    	...
    	if (!AXL::is_logged()) {
        ...
        else if (!$modulo->is_global() && !AXL::has_unidade()) {
    		AXL::force_unidade();
    	}
        else if (!AXL::has_access($modulo)) {
            ...
            $stat_session = DB::getInstance()->verificar_session_id(AXL::get_current_user()->get_id());
            ...
            if ($stat_session == Session::SESSION_ENCERRADA) {
                ...
                header("Location:./?".AXL::K_ERROR.$url);
                exit();
            }
            else if ($stat_session == Session::SESSION_DESATUALIZADA) {
                // refresh session
                $old_user = AXL::get_current_user();
                ...
                Session::getInstance()->setGlobal(AXL::K_CURRENT_USER, $new_user);
            }
            else {
                // set local (no escopo do modulo)
                Session::getInstance()->set(AXL::K_CURRENT_MODULE, $modulo);
    ...
    public static function has_access(Modulo $modulo) {
        ...
        /*if ($id_mod == null) {
            $id_mod = Session::getInstance()->get(AXL::K_CURRENT_MODULE)->get_id();
        }*/
    ...
    public static function check_login($mod_key) {
    	...
    	else if (!AXL::has_unidade()) {
    		AXL::force_unidade();
    	}
        else if (!DB::getInstance()->get_modulo($mod_key, Modulo::MODULO_ATIVO)->get_id()) {
        	$mod = Session::getInstance()->get(AXL::K_CURRENT_MODULE);
        	Template::display_header($modulo->get_nome());
    ...
    public static function is_logged() {
        return Session::getInstance()->exists(AXL::K_CURRENT_USER);
    }

	public static function has_unidade() {
        return AXL::is_logged() && AXL::get_current_user()->get_unidade() != null;
    }
    ...
    public static function get_current_user() {
    	return Session::getInstance()->get(AXL::K_CURRENT_USER);
    }
    ...
    public static function is_module_path($mod_path) {
        $a = explode('.', $mod_path);
        if (sizeof($a) == 2) {
            $path = AXL::K_MODULES_PATH . "/" . $a[0] . "/" . $a[1];
