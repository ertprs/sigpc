...
window.showYesNoDialog = function(onclickok, message, title, onclickcancel) {
    ...
    Ajax.simpleLoad("?dialog=yes_no_dialog", popup.getAttribute("id"), "POST", Ajax.encodePostParameters(p), true, AXL.seleciona,'okbutton');
}
...
window.showErrorDialog = function(message, title, onclickok, onclickcancel) {
    ...
    Ajax.simpleLoad("?dialog=error_dialog", popup.getAttribute("id"), "POST", Ajax.encodePostParameters(p), true, AXL.seleciona,'okbutton');
}
...
window.getParentPopup = function(elem) {
    while (elem['axl_dialog'] == null && elem.parentNode != null) {
        elem = elem.parentNode;
    }

    if (elem['axl_dialog'] != null) {
        return elem['axl_dialog'];
    }
    return null;
}
...
AXL.executaOperacao = function(url, method, params, callbackOk, callbackError) {
...
AXL.addOnLoadListener = function(callback) {
...
AXL.arrayContains = function(array, elem) {
 ...
 * @author robson
 */
AXL.txtBoxSoNumeros = function(objeto, evtKeyPress){
	var nTecla = AXL.getTecla(objeto, evtKeyPress);
 ...
 * @author robson
 */
AXL.getTecla = function (objeto, evtKeyPress){
	var nTecla;
 ...
 * @author robson
 */
AXL.txtBoxAlfaNumerico = function (objeto, evtKeyPress, selecionar){
	var nTecla = AXL.getTecla(objeto, evtKeyPress);
 ...
 * @author robson
 */
AXL.txtBoxAlfa = function (objeto, evtKeyPress, selecionar){
	var nTecla = AXL.getTecla(objeto, evtKeyPress);
 ...
 * @author robson
 */
AXL.onSelecionaSearchType = function (inputBusca, idSelect,selecionar){
	...
	if(select.value == 'login' || select.value == 'codigo'){
		input.onkeypress = function onkeypress(event) {
		    return AXL.txtBoxAlfaNumerico(this,event,selecionar);
		}
	}else{
		input.onkeypress = function onkeypress(event) {
		    return AXL.txtBoxAlfa(this,event,selecionar);
		}
	}
	input.select();
}
 ...
 * @author robson
 */
AXL.adverte = function (label,bool,simbolo){
    if(simbolo == null){
        simbolo = "*";
    }
    if (bool) {
...
AXL.cancelar = function(elem, idPopUp) {
    if (idPopUp == null) {
 ...
 * @author robson
 */
AXL.removerElemSelecionadoSelect = function (idSelect){
	var select = document.getElementById(idSelect);
	if(select.value!=""){
...
AXL.txtBoxFormat = function(objeto, sMask, evtKeyPress) {
...
AXL.alterarConteudo = function(url, onLoad) {
	var func = undefined;
	if (onLoad !== undefined) {
		func = eval(onLoad);
	}
