...
class Template {
	...
	static function display_header($title='', $misc='', $tema_dir = null) {
		...
		<title><?php echo AXL::NAME, ': ', $title; ?></title>
		...
		<script>
            window.onload = AXL.onLoad;
            AXL.addOnLoadListener(new AXL().refresh);
		</script>
		<?php echo $misc;?>
		</head>
		<body>
	...
	public static function display_menu_padrao($modulo, $usuario) {
		if (!Session::getInstance()->exists('MENU')){
			$menu = DB::getInstance()->get_menu(Session::getInstance()->get(AXL::K_CURRENT_MODULE)->get_chave());
	...
	static function display_login_header($label='Enviar', $action='') {
		?>
		<div id="login_header">
			<h1><a href="./" title="Início"><?php echo AXL::NAME; ?></a></h1>
		</div>
		<?php
	}
	...
	static function display_login_form($label='Enviar', $action='', $method='post') {
		...
		<div id="login">
			<form action="<?php echo $action;?>" method="<?php echo $method;?>">
				<h2><?php echo AXL::NAME, ': Acesso'; ?></h2>
	...
	static function display_error($message, $title = 'Erro', $blackout = 0, $onClickOk = '') {
		...
		?>
            <div id="window_popup_icon">
                <img src="themes/axl.default/imgs/dialog-error.png"/>
            </div>
            ...
            <script type="text/javascript">
                AXL.addOnLoadListener(function() {
                    AXL.seleciona("btn_window_error_ok");
                });
            </script>
		<?php
	...
	static function display_input_dialog($message, $title, $callback, $valor='') {
		...
		?>
            <form id ="id_form_window_popup" onsubmit="window.closeInputDialog(this, <?php echo $callback;?>); return false;">
                <div id="window_popup_icon">
                    <img src="themes/axl.default/imgs/dialog-input.png"/>
                </div>
                <div id="window_popup_content">
                	...
                	<div id="window_popup_controls">
                    	...
                    	<input type="button" onclick="location.href='?mod=axl.inicio' " value="Cancelar" />
                	</div>
                </div>
                <script type="text/javascript">
                    AXL.addOnLoadListener(function() {
                        AXL.seleciona("btn_window_input_ok");
                    });
                </script>
            </form>
		<?php
	...
	static function display_confirm_dialog($message, $title = "Informação", $onClickOk = "") {
		...
		?>
            <div id="window_popup_icon">
                <img src="themes/axl.default/imgs/dialog-information.png"/>
            </div>
			<div id="window_popup_content">
	...
	<?php
    /*
     * <input type="text" class="date_field" id="<?php echo $id;?>" name="<?php echo $id;?>" maxlength="10" value="<?php echo $value;?>" onkeypress="return AXL.txtBoxFormat(this, '99/99/9999', event);" onclick="<?php echo $onclick; ?>"/>
	...
	?>
		<h3 class="page_title"><?php echo Session::getInstance()->get(AXL::K_CURRENT_MODULE)->get_nome(), ' > ', $title?></h3>
	<?php
	}
