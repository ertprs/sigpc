...
var CONF_PATH = "?redir=modules/axl/configuracao/";
...
Configuracao.executaReiniciarSenhas = function() {
    ...
    AXL.executaOperacao(CONF_PATH + 'atendimento/exec_reiniciar_senhas.php', 'GET', '', sucesso);
}
...
Configuracao.selectGrupo = function(id_grupo, nm_grupo) {
    ...
    with (elem.style) {
        ...
        background = 'url(themes/axl.default/imgs/arrow_right.png) left center no-repeat';
    }
...
Configuracao.salvarMacroServico = function(input, array_serv, array_id) {
    ...
    if (nm_serv.value==""){
            AXL.adverte(label_nm_serv, true);
            AXL.adverte(label, false);
            nm_serv.select();
            bool = false;
    }
    else if(textBox.value == ""){
        AXL.adverte(label_nm_serv, false);
        AXL.adverte(label,true);
        nm_serv.select();
        bool = false;
    }
        ...
        if (bool) {
            ...
            AXL.adverte(label_nm_serv, true, 'Serviço já existe');
            nm_serv.select();
        }
    }
...
Configuracao.confirmado_salvar_servico = function() {
    ...
    AXL.executaOperacao(CONF_SERVICOS_PATH + "salvar_servico.php", "POST", Ajax.encodeFormAsPost(form), callbackOk);
}
...
//	}
	...
	var old_status = document.getElementById('old_status');
	if (selectServ.value != ""){
		AXL.adverte(selectLabel,false);
		if (textBoxNome.value != ""){
			AXL.adverte(labelNome,false);
			if(textBoxDesc.value != ""){
				AXL.adverte(labelDesc,false);
				...
				AXL.adverte(labelDesc,true);
				textBoxDesc.select();
		//		btn.enabled = true;
			}
		}else{
			AXL.adverte(labelNome,true);
			textBoxNome.select();
		}
	}else{
		AXL.adverte(selectLabel,true);
		selectServ.focus();
	}
}
...
Configuracao.salvarUnidade = function(alterar, array_uni, oldCodUni) {
	...
	if(codUni.value != ""){
				...
				if(input != null){
					if (aux[i]!= input.value){
						if (aux[i]==codUni.value ){
							...
							AXL.adverte(labelCod,true,'Código já existe');
							bool = false;
							break;
						}
					}
		...
		if (bool){
			AXL.adverte(labelCod,false);
		}
		if(nmUni.value != "" && nmUni.value != "digite um nome"){
			AXL.adverte(labelNm,false);
//			if(idGrupo.value != ""){
//				AXL.adverte(labelGrupo,false);
//			}else{
//				idGrupo.focus();
//				AXL.adverte(labelGrupo,true);
//				bool = false;
//			}
		}else{
			if (!codexiste){
