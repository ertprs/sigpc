...
var USUARIOS_PATH = "?redir=modules/axl/usuarios/";
...
Usuarios.removerLotacao = function() {
    ...
    if (nSel == sel_lotacoes.length) {
    	...
    	for(var i = sel_lotacoes.length-1; i >= 0; i--){
    		if(sel_lotacoes.options[i].selected){
    			AXL.removerElemSelecionadoSelect('select_grupos_usuario');
    		}
    	}
    }
 // Atualiza o seletor de unidades para incluir alguma possivel unidade dentro do novo grupo
...
Usuarios.alterarSenhaUsu = function() {
    ...
    AXL.executaOperacao(USUARIOS_PATH + "confirmar_alterar_senha.php", "POST", Ajax.encodePostParameters(p), callbackOk);
}
...
Usuarios.updateUnidadeServicos = function() {
    ...
    for (var i = 0; servsAdd != null && i < servsAdd.length; i++) {
        if (!AXL.arrayContains(servicos, servsAdd[i].value)) {
    ...
    if(select.value != ""){
    	for(var i = select.length-1; i >= 0; i--){
    		if(select.options[i].selected){
    			...
    			AXL.removerElemSelecionadoSelect('select_servicos_atendidos');
    		}
    	}
    }else{
    	select.focus();
...
Usuarios.editarUsuario = function(elem, logins_usu) {
    ...
    for ( i = 0; i < arrayInputs.length-1; i += 2) {
    	if (arrayInputs[i].value == "" && i%2 == 0) {
    		AXL.adverte(arrayInputs[i+1], true);
    		bool = false;
    	}else{
    		AXL.adverte(arrayInputs[i+1], false);
    	}
    }
    ...
    if(bool){
	    ...
	    if (criando) {
	        ...
	        for(i=0;i<logins.length;i++){
	    		if (logins[i] == textLogin.value){
	    			AXL.adverte(labelLogin,true,'Login já existe');
	    			bool = false;
	    			break;
	    		}
	    	}
	        if(senha.length < 6 ){
	        	bool = false;
	        	AXL.adverte(labelSenha1, true);
	        }else{
	        	AXL.adverte(labelSenha1, false);
	        }
	        if(senha != confirmaSenha){
	        	bool = false;
	        	AXL.adverte(labelSenha2, true);
	        }else{
	        	AXL.adverte(labelSenha2, false);
	        }
	    }
	    else {
	        //  editando usuario
	    ...
	    if(bool){
	    	AXL.executaOperacao(USUARIOS_PATH + "gravar_config_usu.php", "POST", Ajax.encodePostParameters(p), callbackOk);
	    }
    }
