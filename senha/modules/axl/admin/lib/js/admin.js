...
var ADM_PATH = "?redir=modules/axl/admin/";
...
Admin.executaReiniciarSenhas = function() {
    ...
    AXL.executaOperacao(ADM_PATH + 'atendimento/exec_reiniciar_senhas.php', 'GET', '', sucesso);
}
/**
...
Admin.criar = function(elem, stat_macro, siglas, editar) {
	...
	if (textSigla != "") {
		AXL.adverte(labelSigla,false);
	} else {
		AXL.adverte(labelSigla,true);
		if(bool){
...
Admin.salvarServ = function(textSigla, textNomeServ, selServico,  statusServ, criar) {
    ...
    AXL.executaOperacao(ADM_MONITOR_PATH + "criar_novo_servico.php", "POST", Ajax.encodePostParameters(parametros), sucesso);
