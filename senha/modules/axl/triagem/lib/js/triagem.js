...
var TRIAG_PATH = "?redir=modules/axl/triagem/";
...
Triagem.distribuir = function(id_servico, id_prio, status_imp) {
    ...
    var callbackOk = function() {
        ...
        location.href = '?mod=axl.triagem';
    }

    AXL.executaOperacao(TRIAG_PATH + "acoes/distribuir_senha.php", "POST", Ajax.encodePostParameters(p), callbackOk)
    ...
    if(radio_serv.checked == true){
        ...
        if(select != null){
            ...
            if(id_atendimento != ""){
                ...
                AXL.adverte(label2, true);
            }
			
        }else{
            AXL.adverte(label1, true);
        }
    }
...
Triagem.confirmaReativarSenha = function(elem) {
    ...
    if(select != null){
        var id_atendimento = select.value;
        if(id_atendimento != ""){
            AXL.adverte(label_senhas, false);
            if (select_prio.value!=""){
                AXL.adverte(label_prioridades, false);
                ...
                select_prio.focus();
                AXL.adverte(label_prioridades, true);
            }
        }else{
            select.focus();
            AXL.adverte(label_senhas, true);
        }
    }
}
...
Triagem.onRadioCancelar = function (radio){
    if (radio.id == "id_radio_servico"){
        ...
        Ajax.simpleLoad(TRIAG_PATH + "acoes/cancelar_senha_por_senha.php", "id_cancelar_senha", "GET", "", false, AXL.seleciona, 'id_text_senha');
    }
...
Triagem.procuraSenha = function (){
    ...
    if(num_senha == ""){
        AXL.seleciona("id_text_senha");
        AXL.adverte(label_senha,true);
    }else{
        AXL.adverte(label_senha,false);
        var parametro = new Object();
