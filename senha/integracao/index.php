...
if(isset($_GET["getStatus"])){
	...
	//echo $condicao;
	$linha = fSQL::SQL_SELECT_ONE("id_uni,id_atend,id_serv,num_senha,num_guiche,nm_cli,id_stat,id_pri","atendimentos",$condicao);
	//print_r($linha);
	if($linha["id_atend"] >= "1"){
		//buscar sigla
		...
		$res["aberto_cont"] = fSQL::SQL_CONTAGEM("painel_senha","num_senha = '".$linha["num_senha"]."' AND id_uni = '".$linha["id_uni"]."' AND id_serv = '".$linha["id_serv"]."' AND num_guiche = '".$linha["num_guiche"]."'");	
	}//if($linha["id"] >= "1"){
